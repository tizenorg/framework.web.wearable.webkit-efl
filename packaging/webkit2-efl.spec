...
License: LGPL-2.1 and BSD-2-Clause and BSD-3-Clause and Apache-1.0 and Apache-1.1 and Apache-2.0
...
%if "%{?tizen_profile_name}" == "wearable"
    ...
    %else
        %define _display_shape RECTANGLE
    %endif
%endif
...
%else
    ...
    %if "%{_repository}" == "emulator" || "%{_repository}" == "emulator-circle"
        %define _build_target "%{_repository}"
        %define _emulator ON
    %else
...
%ifarch %{arm}
    %if "%{model_build_feature_location_position}" == "1"
        %define _geolocation ON
    %else
        %define _geolocation OFF
    %endif
%else
    %define _geolocation ON
%endif
