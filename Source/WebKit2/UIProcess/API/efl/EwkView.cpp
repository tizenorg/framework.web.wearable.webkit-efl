...
#endif
{
    ...
    permissionPopupManager = adoptPtr<PermissionPopupManager>(new PermissionPopupManager(m_evasObject));
...
EwkView* EwkView::create(WKViewRef webView, Evas* canvas, Evas_Smart* smart)
{
    ...
    ewkViewInitializeGeolocationPopup(smartData->priv->evasObject());
#endif
#if ENABLE(TIZEN_NOTIFICATIONS)
    ewkViewInitializeNotificationPopup(smartData->priv->evasObject());
#endif
#if ENABLE(TIZEN_MEDIA_STREAM)
    ewkViewInitializeUserMediaPopup(smartData->priv->evasObject());
#endif
    return smartData->priv;
}
