...
#endif
{
    ...
    if (useDirectRendering) {
        ...
        m_evasGLSurfaceOption = EVAS_GL_OPTIONS_DIRECT;
    } else
...
void EwkView::composite()
{
    ...
    if (useProvidedPixmap()) {
        ecore_evas_extn_socket_lock(ee);
        WKViewPaintToCurrentGLContextWithOptions(wkView(), false, angle);
        ecore_evas_extn_socket_unlock(ee);
    } else
        WKViewPaintToCurrentGLContextWithOptions(wkView(), false, angle);
#else
...
{
#if ENABLE(TIZEN_CHECK_ON_HOLD_EVENT_FLAG)
...
EwkView* EwkView::currentEwkView()
{
    HashMap<WKPageRef, Evas_Object*>::iterator iter = wkPageToEvasObjectMap().begin();
    HashMap<WKPageRef, Evas_Object*>::iterator end = wkPageToEvasObjectMap().end();
    for (; iter != end; ++iter) {
        EwkView* ewkView = toEwkView(iter->value);
        if (WKViewIsVisible(ewkView->wkView()))
            return ewkView;
    }
