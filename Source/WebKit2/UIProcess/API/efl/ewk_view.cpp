...
#if ENABLE(TIZEN_GEOLOCATION)
    EWK_VIEW_IMPL_GET_OR_RETURN(ewkView, impl, false);
    ewk_object_ref(geolocationPermissionRequest);

    GeolocationPermissionPopup* popup = new GeolocationPermissionPopup(geolocationPermissionRequest, ewk_geolocation_permission_request_origin_get(geolocationPermissionRequest), geolocationPermissionText());
...
#if ENABLE(TIZEN_SUPPORT_ITC_UTC)
        EINA_LOG_ERR("Alpha should be between 0 and 255");
#else
...
static Eina_Bool _ewk_view_default_notification_permission(Evas_Object* ewkView, Ewk_Notification_Permission_Request* notificationPermissionRequest, void* userData)
{
    ...
    NotificationPermissionPopup* popup = new NotificationPermissionPopup(notificationPermissionRequest, ewk_notification_permission_request_origin_get(notificationPermissionRequest), notificationPermissionText());
    impl->permissionPopupManager->addPermissionRequest(popup);
#endif
    return true;
}
...
static Eina_Bool _ewk_view_default_user_media_permission(Evas_Object* ewkView, Ewk_User_Media_Permission_Request* userMediaPermissionRequest, void* userData)
{
    ...
    UserMediaPermissionPopup* popup = new UserMediaPermissionPopup(userMediaPermissionRequest, ewk_user_media_permission_request_origin_get(userMediaPermissionRequest), userMediaPermissionText());
    impl->permissionPopupManager->addPermissionRequest(popup);
...
#if ENABLE(TIZEN_GEOLOCATION)
void ewkViewInitializeGeolocationPopup(Evas_Object* ewkView)
{
...
#if ENABLE(TIZEN_NOTIFICATIONS)
void ewkViewInitializeNotificationPopup(Evas_Object* ewkView)
{
...
#if ENABLE(TIZEN_MEDIA_STREAM)
void ewkViewInitializeUserMediaPopup(Evas_Object* ewkView)
{
