...
Ewk_Popup_Picker* ewk_popup_picker_new(Evas_Object* parent, const Eina_List* items, int selectedIndex, Ewk_Popup_Menu* ewk_menu, const Eina_Rectangle* /* rect */)
{
    ...
    Evas_Object* genlist;
    genlist = elm_genlist_add(picker->popup);
    elm_object_part_content_set(picker->popup, "content", genlist);
    ...
    EINA_LIST_FOREACH(items, l, itemv) {
        ...
        Elm_Object_Item* itemObject = elm_genlist_item_append(genlist, itemClass, menuItem, 0, ELM_GENLIST_ITEM_NONE, 0, 0);
    ...
    evas_object_smart_callback_add(genlist, "selected", menuItemActivated, picker);
