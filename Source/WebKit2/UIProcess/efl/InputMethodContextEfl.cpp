...
void InputMethodContextEfl::onIMFInputSequenceComplete(void* data, Ecore_IMF_Context*, void* eventInfo)
{
    ...
    // Selected text should be removed before new text inserted.
    if (inputMethodContext->m_isSelectionRange)
        inputMethodContext->updateApproximateText(String(), inputMethodContext->m_approximateCursorPosition, inputMethodContext->m_selectedLength);
#endif
...
#if ENABLE(TIZEN_SURROUNDING_TEXT_CALLBACK)
    if (!*isFiltered) {
        if (!strcmp(downEvent->keyname, "BackSpace")) {
            if (state.cursorPosition > 0)
                updateApproximateText(String(), state.cursorPosition - 1, 1);
        } else if (!strcmp(downEvent->keyname, "Delete")) {
...
void InputMethodContextEfl::onIMFInputPanelDeleteSurrounding(void* data, Ecore_IMF_Context*, void* eventInfo)
{
    ...
    inputMethodContext->m_approximateCursorPosition = start;
    inputMethodContext->m_approximateSurroundingText = String();
#endif
