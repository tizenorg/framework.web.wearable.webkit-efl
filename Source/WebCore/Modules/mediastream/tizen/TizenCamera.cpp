...
void TizenCamera::createCameraControl(MediaStream* stream, PassRefPtr<CreateCameraSuccessCallback> successCallback, PassRefPtr<CameraErrorCallback> errorCallback)
{
    ...
    if (TizenExtensibleAPI::extensibleAPI().cameraControl()
        || (stream->isAudioOnly() && TizenExtensibleAPI::extensibleAPI().audioRecordingControl())) {
        // check security origin.
        SecurityOrigin* origin = m_context->securityOrigin();
        if (!origin->isLocal())
            allowed = false;

        TIZEN_LOGI("#Local origin => %d", origin->isLocal());
    }
    ...
    m_cameraControl = CameraControl::create(m_context, stream);
    ...
    successCallback->handleEvent(m_cameraControl.get());
}
