...
static PassRefPtr<CSSValue> getPositionOffsetValue(RenderStyle* style, CSSPropertyID propertyID, RenderView* renderView)
{
    ...
    if (style->hasOutOfFlowPosition()) {
        if (length.type() == WebCore::Fixed)
            return zoomAdjustedPixelValue(length.value(), style);
        else if (length.isViewportPercentage())
...
static PassRefPtr<CSSPrimitiveValue> percentageOrZoomAdjustedValue(Length length, const RenderStyle* style, RenderView* renderView = 0)
{
...
static PassRefPtr<CSSValueList> getBorderRadiusCornerValues(LengthSize radius, const RenderStyle* style, RenderView* renderView)
{
    ...
    list->append(percentageOrZoomAdjustedValue(radius.width(), style, renderView));
    list->append(percentageOrZoomAdjustedValue(radius.height(), style, renderView));
    return list.release();
}
...
static PassRefPtr<CSSValue> getBorderRadiusCornerValue(LengthSize radius, const RenderStyle* style, RenderView* renderView)
{
    if (radius.width() == radius.height())
        return percentageOrZoomAdjustedValue(radius.width(), style, renderView);
...
#if ENABLE(CSS_SCROLL_SNAP)
static PassRefPtr<CSSValueList> scrollSnapDestination(RenderStyle& style, const LengthSize& destination)
{
    RefPtr<CSSValueList> snapDestinationValue = CSSValueList::createSpaceSeparated();
    snapDestinationValue->append(percentageOrZoomAdjustedValue(destination.width(), &style));
    snapDestinationValue->append(percentageOrZoomAdjustedValue(destination.height(), &style));
    return snapDestinationValue.release();
}

static PassRefPtr<CSSValue> scrollSnapPoints(RenderStyle& style, const ScrollSnapPoints* points)
{
    if (!points)
        return cssValuePool().createIdentifierValue(CSSValueNone);
    ...
    for (auto& point : points->offsets)
        list->append(percentageOrZoomAdjustedValue(point, &style));
    if (points->hasRepeat)
        list->append(cssValuePool().createValue(LengthRepeat::create(percentageOrZoomAdjustedValue(points->repeatOffset, &style))));
    return list.release();
}

static PassRefPtr<CSSValue> scrollSnapCoordinates(RenderStyle& style, const Vector<LengthSize>& coordinates)
{
    ...
    for (auto& coordinate : coordinates) {
        RefPtr<CSSValueList> pair = CSSValueList::createSpaceSeparated();
        pair->append(percentageOrZoomAdjustedValue(coordinate.width(), &style));
        pair->append(percentageOrZoomAdjustedValue(coordinate.height(), &style));
        list->append(pair.release());
    }
