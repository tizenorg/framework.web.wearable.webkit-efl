...
void ScriptController::setCSPFromWRT(bool fromWRT)
{
    JSDOMWindowShell* windowShell = existingWindowShell(mainThreadNormalWorld());
    if (!windowShell)
        return;
    windowShell->window()->setCSPFromWRT(fromWRT);
}

bool ScriptController::isCSPFromWRT()
{
    JSDOMWindowShell* windowShell = existingWindowShell(mainThreadNormalWorld());
    if (!windowShell)
        return false;
    return windowShell->window()->isCSPFromWRT();
}
#endif
