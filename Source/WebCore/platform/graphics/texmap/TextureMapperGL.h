...
private:
    ...
    void applyRotationMatrixIfNeeded(TransformationMatrix& modelViewMatrix);
#endif

#if USE(TIZEN_PLATFORM_SURFACE)
    void drawTexturedQuadWithProgram(TextureMapperShaderProgram*, uint32_t texture, Flags, const IntSize&, const FloatRect&, const TransformationMatrix& modelViewMatrix, float opacity, const IntSize& platformSurfaceSize);
#endif
