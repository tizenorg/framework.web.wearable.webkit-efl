...
{
#if ENABLE(TIZEN_DIRECT_RENDERING)
    m_context3D = GraphicsContext3DEvasGl::createForCurrentGLContext();
#else
...
#if ENABLE(TIZEN_DIRECT_RENDERING)
    applyRotationMatrixIfNeeded(matrix);
#endif
...
#if USE(TIZEN_PLATFORM_SURFACE)
void TextureMapperGL::drawTexturedQuadWithProgram(TextureMapperShaderProgram* program, uint32_t texture, Flags flags, const IntSize& size, const FloatRect& rect, const TransformationMatrix& modelViewMatrix, float opacity, const IntSize& platformSurfaceSize)
{
    m_context3D->useProgram(program->programID());
    ...
    if (flags & ShouldScaleTexture)
        patternTransform.scaleNonUniform(static_cast<double>(size.width()) / static_cast<double>(platformSurfaceSize.width()),
                                         static_cast<double>(size.height()) / static_cast<double>(platformSurfaceSize.height()));
...
#if ENABLE(TIZEN_DIRECT_RENDERING)
    if (data().PaintFlags & Painting90DegreesRotated || data().PaintFlags & Painting270DegreesRotated)
        data().projectionMatrix = createProjectionMatrix(viewportSize.transposedSize(), data().PaintFlags & PaintingMirrored);
    else
#endif
...
void TextureMapperGL::drawTexture(Platform3DObject texture, Flags flags, const IntSize& textureSize, const FloatRect& targetRect, const TransformationMatrix& modelViewMatrix, float opacity, const IntSize& platformSurfaceSize, bool, unsigned exposedEdges)
{
    if (clipStack().isCurrentScissorBoxEmpty())
        return;

    TransformationMatrix matrix = TransformationMatrix(modelViewMatrix).multiply(TransformationMatrix(
    ...
    if (!transformedTarget.intersects(FloatRect(0, 0, data().viewport[2], data().viewport[3])))
        return;
    ...
    drawTexturedQuadWithProgram(program.get(), texture, flags, textureSize, targetRect, modelViewMatrix, opacity, platformSurfaceSize);
}
#endif

#if ENABLE(TIZEN_DIRECT_RENDERING)
void TextureMapperGL::applyRotationMatrixIfNeeded(TransformationMatrix& modelViewMatrix)
{
    ...
    if (data().PaintFlags & Painting90DegreesRotated) {
        rotationMatrix.rotate3d(0.0, 0.0, 1.0, 270);
        rotationMatrix.translate(-data().viewport[2], 0);
    } else if (data().PaintFlags & Painting270DegreesRotated) {
        rotationMatrix.rotate3d(0.0, 0.0, 1.0, 90);
        rotationMatrix.translate(0, -data().viewport[3]);
    } else
        return;
