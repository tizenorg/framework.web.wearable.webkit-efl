...
void CoordinatedGraphicsLayer::setMaskLayer(GraphicsLayer* layer)
{
    ...
    if (!layer) {
#if ENABLE(TIZEN_FIX_MASK_LAYER_CRASH_ISSUE)
        // We should inform that mask layer has been changed to null.
        // If not, UI process will occurs crash while trying to composite mask layer which is already removed.
...
void CoordinatedGraphicsLayer::setHorizontalScrollbarLayer(CoordinatedLayerID scrollbar)
{
    if (!isScrollable() || m_layerState.horizontalScrollbar == scrollbar)
        return;

    m_layerState.horizontalScrollbar = scrollbar;
    m_layerState.scrollbarChanged = true;
    didChangeLayerState();
}

void CoordinatedGraphicsLayer::setVerticalScrollbarLayer(CoordinatedLayerID scrollbar)
{
    if (!isScrollable() || m_layerState.verticalScrollbar == scrollbar)
        return;

    m_layerState.verticalScrollbar = scrollbar;
    m_layerState.scrollbarChanged = true;
    didChangeLayerState();
}
#endif
...
void CoordinatedGraphicsLayer::setFullScreenLayerForVideo(bool b)
{
    if (fullScreenLayerForVideo() == b)
        return;

    TIZEN_LOGI("FullScreenLayerForVideo : %d", b);

    GraphicsLayer::setFullScreenLayerForVideo(b);
    m_layerState.flagsChanged = true;
...
void CoordinatedGraphicsLayer::syncLayerState()
{
    ...
    if (m_layerState.flagsChanged) {
        ...
        m_layerState.isScrollable = isScrollable();
#if ENABLE(TIZEN_SKIP_COMPOSITING_FOR_FULL_SCREEN_VIDEO)
        m_layerState.fullScreenLayerForVideo = fullScreenLayerForVideo();
#endif
    }
...
void CoordinatedGraphicsLayer::createCanvasIfNeeded()
{
    ...
    if (!m_isValidCanvas) {
        ...
        m_layerState.canvasChanged = true;
#if ENABLE(TIZEN_GSTREAMER_VIDEO) && USE(TIZEN_PLATFORM_SURFACE)
        // Platform surface could be invalid at this function call time because it is created in streaming thread.
        // (See xWindowIdPrepared function of MediaPlayerPrivateGStreamer.)
        // So, we should check validation of surface.
        m_isValidCanvas = m_layerState.canvasToken.isValid();
#else
...
#if ENABLE(TIZEN_FIX_MASK_LAYER_NOT_SHOWING_ISSUE)
    if (maskLayer())
...
void CoordinatedGraphicsLayer::deviceOrPageScaleFactorChanged()
{
    ...
    if (!m_animations.hasActiveAnimationsOfType(AnimatedPropertyWebkitTransform))
        m_fixedAnimationScale = deviceScaleFactor() * pageScaleFactor();
#endif
...
void CoordinatedGraphicsLayer::createBackingStore()
{
    ...
    if (!CoordinatedTileBackendTizen::usePlatformSurface())
        m_mainBackingStore = adoptPtr(new TiledBackingStore(this, CoordinatedTileBackend::create(this)));
    else
#endif
    m_mainBackingStore = adoptPtr(new TiledBackingStore(this, CoordinatedTileBackendTizen::create(this)));
#else
    m_mainBackingStore = adoptPtr(new TiledBackingStore(this, CoordinatedTileBackend::create(this)));
#endif
...
{
#if ENABLE(TIZEN_FIX_FIXED_LAYER_SHAKING_ISSUE)
    if (fixedToViewport() || isIntegral(effectiveContentsScale())) {
#else
...
#if USE(TIZEN_PLATFORM_SURFACE)
PlatformSurfacePoolTizen* CoordinatedGraphicsLayer::platformSurfacePool()
{
    return m_coordinator->platformSurfacePool();
}

void CoordinatedGraphicsLayer::freePlatformSurface(int /*platformSurfaceID*/)
{
    /* FIXME : rebase
    if (m_canvasPlatformLayer)
...
void CoordinatedGraphicsLayer::removePlatformSurface(int /*platformSurfaceID*/)
{
    /* FIXME : rebase
    if (m_canvasPlatformLayer)
...
bool CoordinatedGraphicsLayer::swapPlatformSurfaces()
{
    /* FIXME : rebase
    if (m_canvasPlatformLayer)
...
void CoordinatedGraphicsLayer::flushPlatformSurfaces()
{
    if (m_canvasPlatformLayer)
        m_canvasPlatformLayer->flushPlatformSurfaces();

    for (size_t i = 0; i < children().size(); ++i)
        toCoordinatedGraphicsLayer(children()[i])->flushPlatformSurfaces();
}
#endif

#if ENABLE(TIZEN_WEBKIT2_DEBUG_BORDERS)
bool CoordinatedGraphicsLayer::drawTileInfo() const
{
