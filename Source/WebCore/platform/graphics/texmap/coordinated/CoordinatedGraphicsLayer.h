...
#if USE(TIZEN_PLATFORM_SURFACE)
    virtual PlatformSurfacePoolTizen* platformSurfacePool() = 0;
#endif
};
...
public:
...
#if USE(TIZEN_PLATFORM_SURFACE)
    virtual void setContentsToMedia(PlatformLayer* platformLayer) OVERRIDE { setContentsToCanvas(platformLayer); }
#endif
    ...
    void setHorizontalScrollbarLayer(CoordinatedLayerID);
    void setVerticalScrollbarLayer(CoordinatedLayerID);
#endif
#if ENABLE(TIZEN_SKIP_COMPOSITING_FOR_FULL_SCREEN_VIDEO)
    virtual void setFullScreenLayerForVideo(bool);
#endif
...
#if USE(TIZEN_PLATFORM_SURFACE)
    virtual PlatformSurfacePoolTizen* platformSurfacePool() OVERRIDE;
#endif
    ...
    virtual bool drawTileInfo() const OVERRIDE;
#endif
#endif
