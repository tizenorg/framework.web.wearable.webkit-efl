...
#if ENABLE(TIZEN_WEBKIT2_PRE_RENDERING_WITH_DIRECTIVITY)
    float trajectoryVectorScalar = m_trajectoryVector.length() / trajectoryVectorSpeedRatio;
...
IntSize TiledBackingStore::computeAppropriateTileSize(const IntRect& contentsRect, const IntRect& visibleContentsRect)
{
    ...
    int tileDimension = (shorterSide < 320) ? 160 : (shorterSide < 360) ? 192 : 384;
#else
    int tileDimension = (shorterSide < 500) ? 256 : (shorterSide < 800) ? 384 : 512;
#endif

#if ENABLE(TIZEN_TV_PROFILE)
    int maxTileDimension = tileDimension;
#else
    int maxTileDimension = tileDimension * 2;
#endif

    // compute width
    int tileWidth = (contentsRect.width() <= maxTileDimension) ? ((contentsRect.width()+31) >> 5) << 5 : tileDimension;
    ...
    int tileHeight = (contentsRect.height() <= maxTileDimension) ? contentsRect.height() : tileDimension;
...
#if ENABLE(TIZEN_WEARABLE_PROFILE)
    coverRect.inflateX(m_tileSize.width() / 4);
    coverRect.inflateY(m_tileSize.height() / 4);
