...
static const int s_requestRange[] = { 100 * 1024, 200 * 1024, 200 * 1024, 800 * 1024 };
static const int s_initialBufferSize = 100 * 1024; // 100KByte
static const int s_numberOfInitBuffer = 2;
static const int s_maxRetryCount = 20;
#endif
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
        void requestRetryTimerFired(Timer<StreamingClient>*);
#endif
    private:
        WebKitWebSrc* m_src;
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
        Timer<StreamingClient> m_requestRetryTimer;
#endif
};
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    gboolean firstResponseReceived;
    guint requestIndex;
    guint retryCount;
#endif
...
static void webkit_web_src_init(WebKitWebSrc* src)
{
    ...
    // GStreamer to handle.
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    gst_app_src_set_max_bytes(priv->appsrc, s_initialBufferSize);
#else
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    priv->firstResponseReceived = false;
    priv->requestIndex = 0;
    priv->retryCount = 0;
    gst_app_src_set_max_bytes(priv->appsrc, s_initialBufferSize);
#endif
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
static bool webKitWebSrcStart(WebKitWebSrc* src, bool firstRequest)
#else
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    GOwnPtr<gchar> val;
    guint64 startPos = firstRequest ? priv->requestedOffset : priv->offset;
    guint64 requestRange = s_requestRange[priv->requestIndex];

    if (priv->requestIndex >= s_numberOfInitBuffer) {
        guint64 currentMaxBufferSize = gst_app_src_get_max_bytes(priv->appsrc);
        if (currentMaxBufferSize != requestRange) {
            gst_app_src_set_max_bytes(priv->appsrc, requestRange);
            TIZEN_LOGI("Buffer Size[%d] : %lldK -> %lldK", priv->requestIndex, currentMaxBufferSize / 1024, requestRange / 1024);
        }
    }

    if (priv->requestIndex < sizeof(s_requestRange) / sizeof(int) - 1)
        priv->requestIndex++;

    val.set(g_strdup_printf("bytes=%" G_GUINT64_FORMAT "-%" G_GUINT64_FORMAT , startPos, startPos + requestRange - 1));
    request.setHTTPHeaderField("Range", val.get());
    TIZEN_LOGE("Range : %s", val.get());
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    if (!WebSourceChecker::instance().enabled())
        return false;
#endif
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    , m_requestRetryTimer(this, &StreamingClient::requestRetryTimerFired)
#endif
{
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
void StreamingClient::requestRetryTimerFired(Timer<StreamingClient>*)
{
    webKitWebSrcStart(m_src, false);
}
#endif
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
        TIZEN_LOGE("ERROR! Received response: %d", response.httpStatusCode());
#endif
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    TIZEN_LOGI("Received response: %d", response.httpStatusCode());
    if (priv->firstResponseReceived)
        return;
#endif


#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    String contentRange = response.httpHeaderField("Content-Range");
    long long length = 0;
    if (!contentRange.isEmpty()) {
        TIZEN_LOGE("Content-Range => %s", contentRange.utf8().data());
        String fileLength = contentRange.right(contentRange.length() - contentRange.find('/') - 1);
        length = fileLength.toInt64(0);
    }
    if (!length)
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    priv->firstResponseReceived = true;
#endif
}
...
void StreamingClient::didReceiveData(ResourceHandle* handle, const char* data, int length, int)
{
    ...
    static int index = 0;
    index++;
    TIZEN_LOGI("Have => [index : %d] buffer : %x,  %d bytes of data, offset : %lld", index, priv, length, priv->offset);
#endif
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
    TIZEN_LOGE("seek : %d, size : %lld, offset : %lld", priv->seekID, priv->size, priv->offset);
    if (!priv->seekID && priv->size <= priv->offset)
...
void StreamingClient::didFail(ResourceHandle*, const ResourceError& error)
{
    ...
    priv->retryCount++;
    priv->requestIndex = 0;
    if (priv->retryCount < s_maxRetryCount) {
        TIZEN_LOGE("#Retry => %d", priv->retryCount);
        m_requestRetryTimer.startOneShot(1.5);
        return;
    }
#endif
...
#if ENABLE(TIZEN_MINIMIZE_AUDIO_STREAM_BUFFERING)
bool WebSourceChecker::m_webSourceEnabled = false;

WebSourceChecker& WebSourceChecker::instance()
{
    static WebSourceChecker _instance;
    return _instance;
}

void WebSourceChecker::setEnabled(bool enable)
{
    m_webSourceEnabled = enable;
}

bool WebSourceChecker::enabled()
{
    return m_webSourceEnabled;
}
#endif
