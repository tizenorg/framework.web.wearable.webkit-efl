...
bool LocalMediaServer::createPipeline()
{
    ...
    IntSize maxCaptureSize = IntSize(cameraWidth , cameraHeight);
    if (cameraDeviceCapabilities.supportedCaptureResolutions().size() > 0)
        maxCaptureSize = cameraDeviceCapabilities.supportedCaptureResolutions().last();
...
bool LocalMediaServer::audioStreamIsAvailable() const
{
    ...
    GstElement* audioSrc = gst_element_factory_make("pulsesrc", 0);
